// FxEngine template - base class for delay-line-based reverbs/diffusers.
...
struct DataType<FORMAT_32_BIT> {
  typedef float T;
  static inline float Decompress(T value) { return value; }
...
class FxEngine {
 public:
  ...
  template<int32_t l, typename T_ = Empty>
  struct Reserve {
    typedef T_ Tail;
    enum { length = l };
  };
  ...
  class Context {
   ...
   public:
    ...
    inline void Load(float v) { accumulator_ = v; }
    inline void Read(float v, float scale) { accumulator_ += v * scale; }
    inline void Read(float v) { accumulator_ += v; }
    inline void Write(float& v) { v = accumulator_; }
    inline void Write(float& v, float scale) {
      v = accumulator_;
      accumulator_ *= scale;
    }
    ...
    template<typename D>
    inline void Write(D& d, float scale) { Write(d, 0, scale); }
    ...
    template<typename D>
    inline void Read(D& d, float scale) { Read(d, 0, scale); }
    ...
    inline void Interpolate(
        D&, float offset, LFOIndex index, float amplitude, float scale) {
      STATIC_ASSERT(D::base + D::length <= size, delay_memory_full);
  ...
  inline void Start(Context* c) {
    --write_ptr_;
    if (write_ptr_ < 0) write_ptr_ += size;
    c->accumulator_   = 0.0f;
...
#endif  // WARPS_DRONE_DSP_FX_ENGINE_H_
