...
#
# After this, future updates can be done via the WAV file without JTAG.
#
# Run from repo root after BOTH:
#   TOOLCHAIN_PATH=... make -f warps/bootloader/makefile
#   TOOLCHAIN_PATH=... make -f warps_reverb/makefile build/warps_reverb/warps_reverb_bootloader_combo.bin
set -euo pipefail

OCD="$HOME/opt/xpack-openocd-0.12.0-3/bin/openocd"
SCRIPTS="$HOME/opt/xpack-openocd-0.12.0-3/openocd/scripts"
COMBO="build/warps_reverb/warps_reverb_bootloader_combo.bin"

if [[ ! -f "$COMBO" ]]; then
  echo "no combo binary at $COMBO" >&2
  echo "run: make -f warps_reverb/makefile $COMBO" >&2
  exit 1
fi
